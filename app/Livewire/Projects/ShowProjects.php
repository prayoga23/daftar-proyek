...
class ShowProjects extends Component
{
    ...
    public function toggleHighlight($projectId)
    {
        ...
        if ($project) {
            // Get all projects with the same nama_proyek
            $relatedProjects = Project::where('nama_proyek', $project->nama_proyek)->get();
            
            // Check current status - if any is highlighted, unhighlight all; otherwise highlight all
            $anyHighlighted = $relatedProjects->contains('highlighted', true);
            $newStatus = !$anyHighlighted;
            
            // Toggle the highlighted status for ALL projects with same nama_proyek
            Project::where('nama_proyek', $project->nama_proyek)
                ->update(['highlighted' => $newStatus]);

            // Update the local array with all related project IDs
            $relatedIds = $relatedProjects->pluck('id')->toArray();
            
            if ($newStatus) {
                // Add all related IDs to highlighted array
                $this->highlightedProjects = array_unique(array_merge($this->highlightedProjects, $relatedIds));
            } else {
                // Remove all related IDs from highlighted array
                $this->highlightedProjects = array_values(array_filter($this->highlightedProjects, function($id) use ($relatedIds) {
                    return !in_array($id, $relatedIds);
                }));
            }
        }
    }
